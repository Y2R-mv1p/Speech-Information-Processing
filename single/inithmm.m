...
%     for i = 1:N
%         % 均匀分配状态转移概率
%         hmm.trans(i,:) = [zeros(1,i-1) 0.5 0.5 zeros(1,N-i-1)];
%         if i == N
%             hmm.trans(i,:) = [zeros(1,N-1) 1];
%         end
% 
%         % 简化混合高斯模型初始化
%         for m = 1:M(i)
%             hmm.mix(i).mean(m,:) = randn(1,size(obs(1).fea,1));
%             hmm.mix(i).var(m,:) = ones(1,size(obs(1).fea,1));
%             hmm.mix(i).weight(m) = 1/M(i);
%         end
%     end
%     hmm.trans = mk_stochastic(hmm.trans);
% end
